  ...
  try {
    ...
    if (req.method === "POST" && (path === "api-signals" || path === "")) {
      ...
      for (const item of items) {
        if (!item.platform || !item.raw_title) {
          continue;
        }
        // Normalize title
        ...
        // Duplicate check by raw_title + platform + source_type
      ...
      return json({ ok: true, inserted_count: inserted.length, ids: inserted });
    }
